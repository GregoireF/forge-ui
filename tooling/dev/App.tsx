...
function StatusCard({
  ...
  const time =
    readyMs == null ? "" : readyMs < 1000 ? `${readyMs}ms` : `${(readyMs / 1000).toFixed(1)}s`;
  ...
  return (
    <Box flexGrow={1} justifyContent="center" gap={1}>
      {dot}
      <Text color={meta.color as Parameters<typeof Text>[0]["color"]} bold>
        {meta.label}
      </Text>
...
function FilterTab({ label, active, error }: { label: string; active: boolean; error?: boolean }) {
  ...
  return (
    <Text bold={active} color={fg} backgroundColor={bg}>
      {label}
    </Text>
  );
}
...
export function App() {
  ...
  const errorsByPlayground = useMemo(() => {
    ...
    for (const log of logs) {
      if (log.isError) counts[log.source]++;
    }
    return counts;
  }, [logs]);
  ...
  return (
    <Box flexDirection="column">
      ...
      <Box paddingX={1} gap={1}>
        {searchMode || searchQuery ? (
          ...
          <>
            ...
            <Text dimColor>
              {searchQuery
                ? `  ${allVisible.length} match${allVisible.length !== 1 ? "es" : ""}`
                : ""}
      ...
      <Box flexDirection="column" paddingX={1}>
        {visible.map((log) => (
          <LogRow key={log.id} log={log} textWidth={textWidth} />
        ))}
      </Box>
    </Box>
  );
