...
test.describe("Select — React (forge-ui)", () => {
  ...
  test("ArrowUp opens and highlights last option", async ({ page }) => {
    ...
    // Last non-disabled option should be highlighted
  ...
  test("multi-select: stays open after selecting an item", async ({ page }) => {
    ...
    const firstOption = page
      .locator('[data-forge-scope="select"][data-forge-part="option"]')
  ...
  test("content appears at a non-zero position (no flash to 0,0)", async ({ page }) => {
    ...
    // The positioner must be positioned next to the trigger, not at the top-left corner
