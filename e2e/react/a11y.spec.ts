...
test.describe("Accessibility (axe-core) — React playground", () => {
  ...
  test("Tabs selected: no a11y violations", async ({ page }) => {
    await page.locator('[data-testid="tabs-trigger-react"]').click();
  ...
  test("Popover open: no a11y violations", async ({ page }) => {
    ...
    await page
      .locator('[data-forge-part="content"][data-forge-scope="popover"]')
      .first()
  ...
  test("HoverCard open: no a11y violations", async ({ page }) => {
    await page
      .locator('[data-forge-scope="hover-card"][data-forge-part="trigger"]')
      .first()
      .hover();
    await page
      .locator('[data-forge-scope="hover-card"][data-forge-part="content"]')
      .first()
  ...
  test("Collapsible open: no a11y violations", async ({ page }) => {
    ...
    await page
      .locator('[data-forge-scope="collapsible"][data-forge-part="content"]')
      .first()
      .waitFor({ state: "visible" });
    const results = await new AxeBuilder({ page }).withTags(["wcag2a", "wcag2aa"]).analyze();
    expect(results.violations).toEqual([]);
  ...
  test("Field with validation error: no a11y violations", async ({ page }) => {
    const emailInput = page
      .locator('[data-forge-scope="field"][data-forge-part="control"]')
      .first();
  ...
  test("Menu open: no a11y violations", async ({ page }) => {
    ...
    await trigger.click();
    await page
      .locator('[data-forge-scope="menu"][data-forge-part="content"]')
      .first()
      .waitFor({ state: "visible" });
    const results = await new AxeBuilder({ page }).withTags(["wcag2a", "wcag2aa"]).analyze();
    expect(results.violations).toEqual([]);
  ...
  test("Avatar (idle + error fallbacks visible): no a11y violations", async ({ page }) => {
    await page
      .locator('[data-forge-scope="avatar"][data-forge-part="root"]')
      .first()
      .waitFor({ state: "visible" });
    const results = await new AxeBuilder({ page })
