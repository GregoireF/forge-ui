...
test.describe("Menu (DropdownMenu) — React (forge-ui)", () => {
  ...
  test("checked radio item has aria-checked=true", async ({ page }) => {
    await menuTrigger(page).click();
    // "system" is the default checked radio item
...
test.describe("ContextMenu — React (forge-ui)", () => {
  test.beforeEach(async ({ page }) => {
    await page.goto(URL);
  });
  ...
  test("context menu is hidden on load", async ({ page }) => {
    // No context menu content should be visible
