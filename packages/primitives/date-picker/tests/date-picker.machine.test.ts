...
function make(opts: Parameters<typeof createDatePickerMachine>[0] = {}) {
  const m = createDatePickerMachine({ id: "test", ...opts });
  m.start();
  active.push(m);
  return m;
}
...
describe("createDatePickerMachine — initial state", () => {
  ...
  it("locale defaults to 'en'", () => {
    const m = make();
    expect(m.getSnapshot().context.locale).toBe("en");
  });

  it("custom locale is stored", () => {
    const m = make({ locale: "fr" });
    expect(m.getSnapshot().context.locale).toBe("fr");
  });

  it("firstDayOfWeek defaults to 0 (Sunday)", () => {
    const m = make();
    expect(m.getSnapshot().context.firstDayOfWeek).toBe(0);
  });
  ...
  it("id fallback to 'root' when not provided", () => {
    const m = createDatePickerMachine({});
    m.start();
    active.push(m);
...
describe("createDatePickerMachine — OPEN / CLOSE", () => {
  it("OPEN transitions to open state", () => {
    const m = make();
    m.send("OPEN");
  ...
  it("ESCAPE_KEY closes when open", () => {
    const m = make();
    m.send("OPEN");
    m.send("ESCAPE_KEY");
    expect(m.getSnapshot().matches("closed")).toBe(true);
  ...
  it("INTERACT_OUTSIDE closes when open", () => {
    const m = make();
    m.send("OPEN");
...
describe("createDatePickerMachine — SELECT_FOCUSED", () => {
  ...
  it("does NOT select when focusedDate is unavailable", () => {
    // Start with valid jan15, then move focus to unavailable jun15 via FOCUS_DAY
    // (FOCUS_DAY doesn't validate — SELECT_FOCUSED must guard)
    const m = make({ defaultValue: jan15, isDateUnavailable: (d) => d.month === 6 });
    m.send("OPEN");
    m.send({ type: "FOCUS_DAY", date: jun15 });
    m.send("SELECT_FOCUSED");
...
describe("createDatePickerMachine — month navigation", () => {
  it("NAVIGATE_PREV_MONTH moves focusedDate back one month", () => {
    ...
    expect(m.getSnapshot().context.focusedDate.year).toBe(2024);
  });
...
describe("createDatePickerMachine — focus navigation", () => {
  ...
  it("FOCUS_NEXT_DAY does not cross max boundary (stays put)", () => {
    const m = make({ defaultValue: jan15, max: jan15 });
    m.send("OPEN");
    m.send("FOCUS_NEXT_DAY");
    // Should stay on jan15 since next day is beyond max
    expect(m.getSnapshot().context.focusedDate).toEqual(jan15);
  });

  it("FOCUS_PREV_DAY does not cross min boundary (stays put)", () => {
...
// ---------------------------------------------------------------------------
// FOCUS_WEEK_START / FOCUS_WEEK_END (WAI-ARIA §3.4: Home/End keys)
// ---------------------------------------------------------------------------

describe("createDatePickerMachine — week start/end focus", () => {
  it("FOCUS_WEEK_START moves to Sunday of current week (Sunday-first)", () => {
    // 2024-06-15 is a Saturday (6). Sunday-first: back 6 days = June 9 (Sunday)
    const m = make({ defaultValue: jun15 });
    m.send("OPEN");
  ...
  it("FOCUS_WEEK_END moves to Saturday of current week (Sunday-first)", () => {
    // 2024-06-15 is Saturday → it IS the end of the week already
    const m = make({ defaultValue: jun15 });
    m.send("OPEN");
  ...
  it("FOCUS_WEEK_START with Monday-first moves to Monday of current week", () => {
    // June 15, 2024 = Saturday. Monday-first: back 5 days = June 10 (Monday)
...
describe("createDatePickerMachine — SET_VALUE", () => {
  ...
  it("focusedDate unchanged when SET_VALUE with undefined", () => {
    const m = make({ defaultValue: jan15 });
    const initialFocused = m.getSnapshot().context.focusedDate;
    m.send({ type: "SET_VALUE", date: undefined });
    expect(m.getSnapshot().context.focusedDate).toEqual(initialFocused);
  });
...
// ---------------------------------------------------------------------------
// optional props stored in context
// ---------------------------------------------------------------------------

describe("createDatePickerMachine — optional context fields", () => {
  it("min stored when provided", () => {
    const m = make({ min: jan15 });
    expect(m.getSnapshot().context.min).toEqual(jan15);
  });

  it("min absent when not provided", () => {
    const m = make();
    expect(m.getSnapshot().context.min).toBeUndefined();
  });

  it("max stored when provided", () => {
    const m = make({ max: jun15 });
    expect(m.getSnapshot().context.max).toEqual(jun15);
  });

  it("isDateUnavailable stored when provided", () => {
    const fn = vi.fn();
    const m = make({ isDateUnavailable: fn });
    expect(m.getSnapshot().context.isDateUnavailable).toBe(fn);
  });

  it("onValueChange stored when provided", () => {
    const fn = vi.fn();
    const m = make({ onValueChange: fn });
    expect(m.getSnapshot().context.onValueChange).toBe(fn);
  });

  it("onOpenChange stored when provided", () => {
    const fn = vi.fn();
    const m = make({ onOpenChange: fn });
    expect(m.getSnapshot().context.onOpenChange).toBe(fn);
  });
});
